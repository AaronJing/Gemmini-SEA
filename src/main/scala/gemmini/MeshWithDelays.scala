...
class MeshWithDelays[T <: Data: Arithmetic, U <: TagQueueTag with Data]
  ...
  val mesh = if (!sea) Module(new Mesh(inputType, outputType, accType, df, tree_reduction, tile_latency, max_simultaneous_matmuls, output_delay, tileRows, tileColumns, meshRows, meshColumns))
else Module(new Mesh_sea_ws(samesigned, approxmul, no_round, inputType, outputType, accType, df, tree_reduction, tile_latency, max_simultaneous_matmuls, output_delay, tileRows, tileColumns, meshRows, meshColumns))
