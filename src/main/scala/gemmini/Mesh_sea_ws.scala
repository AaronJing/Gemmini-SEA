...
/**
  ...
  * @param meshColumns
  */
trait MeshIntf[T <: Data] extends Module {
  // Abstract definitions for the members of the interface

    def in_a: Vec[Vec[T]]                  
    def in_b: Vec[Vec[T]]                  
    def in_d: Vec[Vec[T]]                  
    def in_control: Vec[Vec[PEControl[T]]] 
    def in_id: Vec[Vec[UInt]]              
    def in_last: Vec[Vec[Bool]]            
    def out_b: Vec[Vec[T]]                 
    def out_c: Vec[Vec[T]]                 
    def in_valid: Vec[Vec[Bool]]           
    def out_valid: Vec[Vec[Bool]]          
    def out_control: Vec[Vec[PEControl[T]]]
    def out_id: Vec[Vec[UInt]]             
    def out_last: Vec[Vec[Bool]]           
}
class Mesh_sea_ws[T <: Data](samesignedadder: Boolean, approx: Boolean, noround: Boolean,inputType: T, outputType: T, accType: T,
                                   ...
                                   val meshRows: Int, val meshColumns: Int)(implicit ev: Arithmetic[T])  extends Module with MeshIntf[T]  {
  ...
  for (c <- 0 until meshColumns) {
    ...
    // get postive/negative zero for in_b1 if in_b is negative/postive respectively
    val zeros = VecInit(Seq.fill(tileColumns)((0.U((outputType.getWidth).W).asTypeOf(outputType))))
    for(tc <- 0 until tileColumns){
      zeros(tc) := (Cat(~(((in_b(c))(tc)).isNeg), 0.U((outputType.getWidth-1).W))).asTypeOf(outputType)
    }
    meshT(c).foldLeft((zeros, in_valid(c))) {
      case ((in_b, valid), tile) =>
