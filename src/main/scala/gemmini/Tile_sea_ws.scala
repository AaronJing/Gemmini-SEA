...
/**
  ...
  */
class Tile_sea_ws[T <: Data](samesignedadder: Boolean, approx: Boolean, noround: Boolean, inputType: T, outputType: T, accType: T, df: Dataflow.Value, tree_reduction: Boolean, max_simultaneous_matmuls: Int, val rows: Int, val columns: Int)(implicit ev: Arithmetic[T]) extends Module {
  val io = IO(new Bundle {
    val in_a        = Input(Vec(rows, inputType))
    val in_b        = Input(Vec(columns, outputType)) // This is the output of the tile next to it
    val in_b1        = Input(Vec(columns, outputType)) // This is the output of the tile next to it
    ...
    val out_b1       = Output(Vec(columns, outputType))
  ...
  val tile = Seq.fill(rows, columns)(Module(new PE_sea_ws(samesignedadder, approx, noround, inputType, outputType, accType, df, max_simultaneous_matmuls)))
